...
const checks = [
  ...
  ['pending contact uses sessionStorage and never localStorage', /sessionStorage\.setItem\(PENDING_PHONE_KEY/.test(authStore) && !/localStorage[\s\S]*phone/i.test(runtime)],
  ['password is never persisted with pending registration', !/(localStorage|sessionStorage)\.setItem\([^,]+,\s*password/i.test(runtime)],
  ['phone is editable in own profile', /saveMyPrivateContact/.test(contactService) && /phone: normalizePhone\(phone\)/.test(profileForm)],
